...
start_time = time.time() # Timing the model run
...
initial_temp = 1200  #Initial temperature of the system (Kelvin)
temp_drop = 150  #Temperature drop over the total simulation time (Kelvin)

# Simulation parameters
inclusion_radius = 50e-6  #Inclusion radius (meters)
host_size = 0.5e-3  #Host radius (meters)
num_steps = 16000  #Number of spatial steps
...
def analytical_solution_adjusted(x, t, D0, Ea, R, initial_temp, temp_drop, total_time, C0):
    ...
    num_integral_steps = 16000  # Resolution of numerical integration
...
# Function to simulate diffusion using the Crank-Nicolson method
def run_simulation(num_steps, dx, dt):
    x_values = np.linspace(0, host_size, num_steps + 1)
    concentration = np.zeros(num_steps + 1)
    concentration[x_values <= inclusion_radius] = C0

    profiles = [concentration.copy()]
    profile_times = [0]

    for step in range(num_time_steps):
        current_time = step * dt
        current_temp = initial_temp - (temp_drop * current_time / total_time)
        D = D0 * np.exp(-Ea / (R * current_temp))
        r = D * dt / dx**2

        a = -r / 2 * np.ones(num_steps + 1)
        b = (1 + r) * np.ones(num_steps + 1)
        c = -r / 2 * np.ones(num_steps + 1)
        ...
        concentration = solve_tridiagonal(a, b, c, rhs)

        if current_time in observation_times:
            profiles.append(concentration.copy())
            profile_times.append(current_time)

    return x_values, profiles, profile_times
...
for time_analytical in times_coarse:
